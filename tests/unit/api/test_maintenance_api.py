...
def test_set_maintenance_status(flask_app, mocker: MockFixture):
    ...
    response = flask_app.post(url_for("maintenance_api.set_status"), json=MAINTENANCE_STATUS_REQUEST_VALID)
...
def test_set_maintenance_status_with_no_msg(flask_app, mocker: MockFixture):
    db_set_status = mocker.patch.object(maintenance_db, "set_status")
    response = flask_app.post(url_for("maintenance_api.set_status"), json=MAINTENANCE_STATUS_REQUEST_NO_MSG)
...
def test_set_maintenance_status_with_invalid_pause(flask_app, mocker: MockFixture):
    db_set_status = mocker.patch.object(maintenance_db, "set_status")
    response = flask_app.post(url_for("maintenance_api.set_status"), json=MAINTENANCE_STATUS_REQUEST_INVALID_PAUSE_VALUE)
...
def test_get_maintenance_status(flask_app, mocker: MockFixture):
    get_status = mocker.patch.object(maintenance_db, "get_latest_status", return_value=RESPONSE_FROM_DB[0])
...
def test_get_maintenance_history(flask_app, mocker: MockFixture):
    get_history = mocker.patch.object(maintenance_db, "get_history", return_value=RESPONSE_FROM_DB)
